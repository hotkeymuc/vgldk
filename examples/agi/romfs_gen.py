...
class ROMFS_File:
	def __init__(self, filename, data=b'', id=None, org_filename=None, align_size=1):
		...
		self.data_offset = 0	# Absolute data offset (always starts at 0)
		self.file_offset = 0	# Where in the ROM binary file the data starts
		self.chip_offset = 0	# Where inside the physical ROM chip the data starts (usually at the same offset as inside the ROM file)
		self.mem_offset = 0	# Where in memory the data starts (depends where it is bank switched to...)
		self.mem_bank = 0
...
class ROMFS:
	def __init__(self, file_offset=0, chip_offset=0, mem_offset=0, mem_bank_start=0, mem_bank_size=0x1000):
		self.files = []
		self.data_offset = 0	# Absolute data offset (always starts at 0)
		self.file_offset = file_offset	# Where in the ROM binary file the data starts
	...
	def layout(self):
		...
		for f in self.files:
			# Align
			align = 0
			#if o % f.align_size > 0:	# Non-aligned!
			if o_chip % f.align_size > 0:	# Non-aligned!
				o_align = ((o_chip // f.align_size) + 1) * f.align_size
				align = o_align - o_chip
				#put(f'	* Aligning "{f.filename}" on 0x{f.align_size:04X} byte segments: From 0x{o:04X} to 0x{o_align:04X} ({align} bytes wasted)')
			
			# Add the alignment padding
			#o += align	# Do not align absolute data
			o_file += align
			o_chip += align
			o_mem += align
	...
	def dump_stats(self):
		...
		r += f"{'num'} {'filename':<{filename_size}} {'id':<{id_size}} {'offset':>8} {'file ofs':>8} {'chip ofs':>8} {'bank':>4} {'addr':>6} {'size':>8}\n"
		r += f"{'':-<3} {'':-<{filename_size}} {'':-<{id_size}} {'':->8} {'':->8} {'':->8} {'':->4} {'':->6} {'':->8}\n"
		
		# Each file...
		total_data_size = 0
		aligned_size = 0
		aligned_waste = 0
		o_previous = self.data_offset
		for i,f in enumerate(self.files):
			#r += f"{i:>3} {f.id:<{id_size}} {f.filename:<{filename_size}} 0x{f.data_offset:06X} {f.size:>8,}\n"
			r += f"{i:>3} {f.filename:<{filename_size}} {f.id:<{id_size}} 0x{f.data_offset:06X} 0x{f.file_offset:06X} 0x{f.chip_offset:06X} 0x{f.mem_bank:02X} 0x{f.mem_offset:04X} {f.size:>8,}\n"
			
			total_data_size += f.size
			aligned_size = max(aligned_size, f.data_offset + f.size)
			aligned_waste += (f.data_offset - o_previous)
			o_previous = f.data_offset + f.size
			
		r += f"{'':=<3} {'':=<{filename_size}} {'':=<{id_size}} {'':=>8} {'':=>8} {'':=>8} {'':=>4} {'':=>6} {'':=>8}\n"
		r += f"{len(self.files):>3} {'final':<{filename_size}} {'':<{id_size}} 0x{self.final_data_offset:06X} 0x{self.final_file_offset:06X} 0x{self.final_chip_offset:06X} 0x{self.final_mem_bank:02X} 0x{self.final_mem_offset % self.mem_bank_size:04X} {total_data_size:>8,}\n"
		
		r += '\n'
		r += f'Aligned size : 0x{aligned_size:06X} / {aligned_size:>8,} bytes / {aligned_size/1024:6.1f} KB\n'
