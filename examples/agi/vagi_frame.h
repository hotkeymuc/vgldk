...
// Frame functions (handling one full resolution AGI frame)
	...
	void frame_clear(byte b) {
		...
		memset((byte *)AGI_FRAME_ADDR, b, (AGI_FRAME_HEIGHT * (AGI_FRAME_WIDTH >> 1)));
	}
	...
	void frame_contiguous_set_pixel_4bit(byte x, byte y, byte c) {
		...
		a = AGI_FRAME_ADDR + y * (AGI_FRAME_WIDTH >> 1) + (x >> 1);
	...
	void frame_clear(byte b) {
		...
		memset((byte *)AGI_FRAME_ADDR, b, 0x2000);	// Clear whole first buffer
		bank_0xc000_port = AGI_FRAME_BANK_HI;	// Mount bank 2 to 0xc000
