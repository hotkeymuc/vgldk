...
#define speech_set_data(d) speech_port_data = (d << 4)
...
byte speech_get_data() {
	...
	*/
	...
	// Reverse
	//d = ((d & 0xcc) >>2) | ((d & 0x33) <<2);	// Swap in groups of 2
	//d = ((d & 0xaa) >>1) | ((d & 0x55) <<1);	// Swap bit pairs
	...
	while(1) {
		...
		switch(c) {
			...
			case '0':
				...
				speech_play((word)(&spAFFIRMATIVE), 180);
				break;
