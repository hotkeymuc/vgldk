...
// HtK
// Used in VGLDK's host_mame.h
uint8_t pc2000_state::debug_r()
{
	//return debug_port_r(0x13 + offset);
	
	//@TODO: Use FIFO to read from other process
	//printf("r");
	//return 0x55;
...
void pc2000_state::debug_w(uint8_t data)
{
	//@TODO: Use FIFO to pass on to other process
	printf("%02X\n", data);
	
	//debug_port_w(0x13 + offset, data);
}
...
void pc2000_state::pc2000_mem(address_map &map)
{
	...
	map(0x0000, 0x3fff).bankrw("bank0");	// Needed to test CP/M (RAM at ROM address)
	map(0x4000, 0x7fff).bankrw("bank1");	// Needed to test CP/M (RAM at ROM address)
