/*
	...
	@FIXME: There seems to be a severe compiler bug
	! When calling _sin(int) it will return correctly sometimes, but sometimes return 0xff
...
void drawScreen() {
	...
	for(ix = 0; ix < SCREEN_W; ix++) {
		...
		dx = _sin(a);
		//dz = _sin((a + SINTABLE_SIZE + SINTABLE_SIZE/4) % SINTABLE_SIZE);
		dz = _cos(a);
		...
		for(iz = 0; iz < MAX_DEPTH*OVERSAMPLE_RAY; iz++) {
			...
			x = player_x;
			x += (OVERSAMPLE_POS * dx * iz) / (SINTABLE_SCALE*OVERSAMPLE_RAY);
			x /= OVERSAMPLE_POS;
			
			z = player_z;
			z += (OVERSAMPLE_POS * dz * iz) / (SINTABLE_SCALE*OVERSAMPLE_RAY);
			z /= OVERSAMPLE_POS;
...
void main() __naked {
	...
	player_x = (OVERSAMPLE_POS * LEVEL_W) / 2;	// Center
	player_z = (OVERSAMPLE_POS * LEVEL_H) / 2;	// Center
	...
	while(1) {
		...
		*/
		...
		if ((move_a != 0) || (move_z != 0)) {
			...
			/*
			#error "Sanity error: Function did not return the correct value. I do not know what to say or do."
			
			//@FIXME: Compiler bug! sx=255
			if (sx != _sin(player_a)) {
				
				lcd_x = 0; lcd_y = 1;
				puts("a="); printf_d(player_a);
				puts(",sx="); printf_d(sx);
				puts(",sx="); printf_d(_sin(player_a));
				puts(",sz="); printf_d(sz);
