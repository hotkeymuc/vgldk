...
file_DIR fs_bb_tmpDir;
file_DIR *fs_bb_opendir(const char *path) {
	...
	*/
	...
	dir->count = 0;	//(sizeof(FS_INTERNAL_FILES) / sizeof(file_FILE));
	//rewinddir(dir);
	dir->currentPos = 0;
...
int fs_bb_closedir(file_DIR * dir) {
	BB_HANDLE dirHandle;
...
//struct dirent *readdir(file_DIR *dir) {
dirent *fs_bb_readdir(file_DIR *dir) {
	//struct dirent *de;
	dirent *de;
	BB_HANDLE dirHandle;
...
file_FILE fs_bb_tmpFile;
file_FILE *fs_bb_fopen(const char *path, const char *openMode) {
...
size_t fs_bb_fread(void *ptr, size_t size, size_t nmemb, file_FILE *f) {
	// Read SIZE elements of size NMEMB and return how many elements were read
	BB_HANDLE handle;
	byte l;
...
size_t fs_bb_fwrite(void *ptr, size_t size, size_t nmemb, file_FILE *f) {
	BB_HANDLE handle;
	byte l;
	byte *b;
