...
//#define HTK_TMS	// Experiments with speech on gl6000sl/prestige
...
private:
	...
	#ifdef HTK_TMS
		//optional_device<tms5110_device> m_tms5100;	// Optional, because at the moment only gl6000sl has it
		optional_device<tms5220_device> m_tms5220;	// Optional, because at the moment only gl6000sl has it
	...
	uint8_t tsp10_r(offs_t offset);
	void tsp10_w(offs_t offset, uint8_t data);
	uint8_t tsp10_state;
	uint8_t tsp11_state;
	uint8_t tsp21_state;
	
	uint8_t tsp20_r(offs_t offset);
	void tsp20_w(offs_t offset, uint8_t data);
	
	uint8_t tsp43_r(offs_t offset);
	void tsp43_w(offs_t offset, uint8_t data);
	
	uint8_t tsp60_r(offs_t offset);
	void tsp60_w(offs_t offset, uint8_t data);
	uint8_t tsp60_state;
};
...
void prestige_state::bankswitch_w(offs_t offset, uint8_t data)
{
	...
	case 2:
		// 0x8000 - 0xBFFF - default cartridge space
...
// HtK
...
#ifdef HTK_TMS
	// Based on TI Speak&Spell at "tispeak.cpp"
	
	void prestige_state::tms_w(offs_t offset, uint8_t data)
	{
		
		uint8_t p = 0x10+offset;
		...
		*/
		...
		if (p == 0x10) {
			// R10: TMS5100 PDC pin
			//m_tms5100->pdc_w((data >> 2) & 1);	// firmware puts "0x00" and "0x04" into port 0x10
			
			//m_tms5220->combined_rsq_wsq_w((data & 0x04) >> 2);	// firmware puts "0x00" and "0x04" into port 0x10
			m_tms5220->wsq_w(1 - ((data & 0x04) >> 2));	// firmware puts "0x00" and "0x04" into port 0x10
		}
		if (p == 0x11) {
			// O3210: TMS5100 CTL8124
			//m_tms5100->ctl_w(data);	//bitswap<4>(data,3,0,1,2));
			
			m_tms5220->data_w(data);
		}
	}
	
	uint8_t prestige_state::tms_r(offs_t offset)
	{
		uint8_t p = 0x10+offset;
		...
		r = k4 | m_inputs[9]->read() | read_inputs(9);
		*/
		if (p == 0x10) {
			//r = m_tms5100->ctl_r();
			
			r = 0xff - m_tms5220->status_r();
			//r = m_tms5220->intq_r();
			//r = m_tms5220->readyq_r();
		}
	...
	void prestige_state::tms5110_route(machine_config &config)
	{
		/* sound hardware */
		...
		m_tms5220->add_route(ALL_OUTPUTS, "mono", 0.5);
	}
#endif



uint8_t prestige_state::debug_r(offs_t offset)
{
	/*
	long a;
	...
	*/
	...
	// Compatibility with VGLDK:bdos_host
	int i;
	int rv;
	//rv = fscanf(f, "2X", &i);
	
	// Retry until something valid is found
	rv = 0;
	while (rv != 1) {
		rv = scanf("%2X\n", &i);
...
void prestige_state::debug_w(offs_t offset, uint8_t data)
{
	//@TODO: Use FIFO to pass on to other process
	//printf("W%02X=%02X\n", p, data);
...
uint8_t prestige_state::tsp10_r(offs_t offset)
{
	uint8_t p = 0x10+offset;
	...
	if (p == 0x10) {
		r = tsp10_state;	// 0xF9 (idle) / 0xFF/0xFD (running) / 0xFC (stopped) / 0xFF
		//if ((tsp10_state & 7) == 5) tsp10_state = 0xf8 | 1;
		//else tsp10_state = 0xf8 | 5;
		//if (tsp10_state == 0xfd) tsp10_state = 0xff;
		tsp10_state ^= 5;
...
void prestige_state::tsp10_w(offs_t offset, uint8_t data)
{
	...
	if (p == 0x10) {
		if (data == 0x04) {
			tsp10_state = 0xf8 + 5;	//|= 0x04;
			//tsp60_state ^= 0x20;	//|= 0x04;	// When 0x60 turns from 0x0f to 0x20 something magical happens...
			
			//@TODO: Cause NMI(s)
			m_maincpu->set_input_line(INPUT_LINE_NMI, ASSERT_LINE);	//newval ? CLEAR_LINE : ASSERT_LINE);
			
		}
		if (data == 0x00) {
			tsp10_state = 0xf9;	//&= 0xfb;
		}
	} else
	if (p == 0x11) {
		tsp60_state ^= 0xff;	//0x0f;
...
/*
uint8_t prestige_state::tsp20_r(offs_t offset)
{
	uint8_t p = 0x20+offset;
	
	
	uint8_t r;
	r = 0xff;
	if (p == 0x21) r = tsp21_state++;	// Usually 0x7f
	if (p == 0x23) r = 0x60;
	if (p == 0x25) r = 0xf0;
...
void prestige_state::tsp20_w(offs_t offset, uint8_t data)
{
	uint8_t p = 0x20+offset;
...
uint8_t prestige_state::tsp43_r(offs_t offset)
{
	uint8_t p = 0x43+offset;
...
void prestige_state::tsp43_w(offs_t offset, uint8_t data)
{
	uint8_t p = 0x43+offset;
...
uint8_t prestige_state::tsp60_r(offs_t offset)
{
	uint8_t p = 0x60+offset;
	...
	#ifdef HTK_TMS
	if (p == 0x60) {
		//r = tsp60_state++;	// 0x0b / 0x0f / 0x0c	- Stuff happens when it goes to 0x20
		//r = 0x0b | m_tms5100->ctl_r();
		
		//r = m_tms5220->readyq_r();
		//r = (1 - m_tms5220->intq_r()) * 0x20;
		r = m_tms5220->status_r();
		//r = 0x20;
	};
	#else
	if (p == 0x60) {
...
void prestige_state::tsp60_w(offs_t offset, uint8_t data)
{
	//uint8_t p = 0x60+offset;
...
void prestige_state::prestige_io(address_map &map)
{
	...
	// HtK 2022-01-20: Trying to emulate the TSP-like speech synth
	#ifdef HTK_TMS
		map(0x10, 0x11).rw(FUNC(prestige_state::tms_r), FUNC(prestige_state::tms_w));
	#endif
	
	// Used in VGLDK's host_mame.h
	map(0x13, 0x13).rw(FUNC(prestige_state::debug_r), FUNC(prestige_state::debug_w));
	
	tsp10_state = 0xf9;
	tsp11_state = 0xff;
	tsp21_state = 0x00;	// Usually 0x7f;
	tsp60_state = 0x0b;
	map(0x10, 0x11).rw(FUNC(prestige_state::tsp10_r), FUNC(prestige_state::tsp10_w));
	//map(0x20, 0x29).rw(FUNC(prestige_state::tsp20_r), FUNC(prestige_state::tsp20_w));
	// 0x23 := 0x60
	// 0x29 := 0x01
	// 0x43 := 0x02
	// 0x61 := 0xd0
	//map(0x43, 0x43).rw(FUNC(prestige_state::tsp43_r), FUNC(prestige_state::tsp43_w));
	map(0x60, 0x62).rw(FUNC(prestige_state::tsp60_r), FUNC(prestige_state::tsp60_w));
...
void prestige_state::machine_start()
{
	...
	#ifdef HTK_TMS
	//if (m_tms5100 != nullptr)	m_tms5100->reset();
	if (m_tms5220 != nullptr)	m_tms5220->reset();
	#endif
}
...
void prestige_state::gl6000sl(machine_config &config)
{
	prestige_base(config);
		...
		// TMS51xx:
		//TMS5100(config, m_tms5100, MASTER_CLOCK);
		//TMS5100A(config, m_tms5100, MASTER_CLOCK);
		//TMS5110(config, m_tms5100, MASTER_CLOCK);
		...
		//M58817(config, m_tms5100, MASTER_CLOCK);
		
		// TMS52xx:
		//TMS5200(config, m_tms5220, MASTER_CLOCK);
		TMS5220(config, m_tms5220, MASTER_CLOCK);
		//TMS5220C(config, m_tms5220, MASTER_CLOCK);
		//CD2501E(config, m_tms5220, MASTER_CLOCK);
		//CD2501ECD(config, m_tms5220, MASTER_CLOCK);
